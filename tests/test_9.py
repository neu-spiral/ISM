...
fsize = '200'
file_name = 'data_sets/Four_gaussian_3D_' + fsize + '.csv'
orig_label = 'data_sets/Four_gaussian_3D_' + fsize + '_original_label.csv'
alt_label = 'data_sets/Four_gaussian_3D_' + fsize + '_alt_label.csv'


data = genfromtxt(file_name, delimiter=',')	
label_1 = genfromtxt(orig_label, delimiter=',')	
label_2 = genfromtxt(alt_label, delimiter=',')	
...
if True:	#	Alterntive clustering
	...
	ASC.set_values('ISM_exit_count',3)
	...
	print "Alt against orig NMI : " , normalized_mutual_info_score(a,allocation)
	g_truth = np.concatenate((np.ones(100), np.zeros(100),np.ones(100), np.zeros(100)))
	print "Alt against Alt NMI : " , normalized_mutual_info_score(allocation,g_truth)
	b = db['allocation']
