...
def Orthogonal_implementation(db):
	...
	if db['prev_clust'] == 0: 
		...
		elif db['init_W_from_pickle']:						# If we initialize W from some pickle file
			fname = "init_files/init_W_" + db['Experiment_name'] + ".pk"
			if os.path.exists(fname):
				init_W = pickle.load( open( fname, "rb" ) )
				db['W_matrix'] = init_W[db['pickle_count']]

				if db['W_matrix'].shape[1] != db['q']: 
					error_raise('Your saved W matrix has a different dimension')
			else:
				raise
		else:
		...
		while WU_converge == False: 	
			...
			##------------
			#[Lagrange_Gradient, new_grad_mag, constraint_mag] = cf.derivative_test(db)
			#print 'Derivative : ' , Lagrange_Gradient
...
def ISM_implementation(db):
	...
	if db['prev_clust'] == 0: 
		...
		#	Set up Initial W matrix
		if not db['init_W_from_pickle']:	# If we initialize W from some pickle file
			...
			if db['prev_clust'] == 0: 
				...
				if os.path.exists(fname):
					...
					print 'You did not include the init_W file'
					raise
...
def DongLing_implementation(db):
	...
	if db['prev_clust'] == 0: 
		...
		while WU_converge == False: 	
			...
			#print db['cf'].Final_calc_cost(db['W_matrix'])
			#import pdb; pdb.set_trace()


		cf.calc_psi()	# this make sure that cost function are accurate

		#latest_cost = db['cf'].calc_cost_function(db['W_matrix'])
		#print ':::::: ' , latest_cost
		#import pdb; pdb.set_trace()
